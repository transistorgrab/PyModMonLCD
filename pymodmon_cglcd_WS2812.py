...
# requires: Python >3.7, pymodbus, docopt, pillow, spidev, tkinter, RPi.GPIO, Adafruit_ILI9341, adafruit-circuitpython-neopixel
#
...
It also provides the control for WS2812 intelligend LEDs (aka "Neopixel").
...
## provide hardware specifics for neopixel library
import board

## library for control of the Neopixels
import neopixel
...
class LED(object):
    def __init__(self):
        self.led_gpio   = board.D18            ## GPIO where LED strip data input is connected
        self.led_count  = 32            ## Number of LEDs in the strip
        self.brightness = 0.1
        self.leds       = neopixel.NeoPixel(self.led_gpio, self.led_count, brightness=self.brightness)  ## where are the LEDs connected, how many
        self.led_data   = []

    ## displays the configured LEDs
    def display(self):
        self.leds.show()    ## prevent xfer2 from deleting the led_data list

    ## disables LED display
    def disable(self):
        self.leds.fill( (0,0,0) ) ## disables all LEDs
        self.leds.show()
#------------------------- LED class --------------------------------------------------------------
...
## function for writing to LCD and LED
    ...
    def writeLoggerDataLCD(self):
        ...
        ## formats led data
        #  there a 32 LEDs that provide fast overview of load/PV yield
        ## scaling is logarithmic, for values < 1kW we add 100 W to be able to display values smaller 100 W
        if (float(dc_watts) > 1000.0):
            dc_watts_led = float(dc_watts)
        else:
            dc_watts_led = float(dc_watts) * 0.9 + 100

        ## normalize power value if out of bounds
        if (dc_watts_led < 100.0):
            dc_watts_led = 100.0
        if (dc_watts_led > 10000.0):
            dc_watts_led = 10000.0

        ## Same procedure for the load power
        if (load_wa_i > 1000):
            load_watts_led = float(load_wa_i)
        else:
            load_watts_led = float(load_wa_i) * 0.9 + 100

        if (load_watts_led < 100.0):
            load_watts_led = 100.0
        if (load_watts_led > 10000.0):
            load_watts_led = 10000.0

        ## we need to convert watts to kW for easier calculation of log10: log10(1kw)=1
        # 1 kw equals 16 leds, 10 kw = 32 leds, values < 1kw give negative results from log10
        LED_count_green = round(16+(math.log10(dc_watts_led/1000.0)*16))
        LED_count_red   = round(16+(math.log10(load_watts_led/1000.0)*16))

        ## updates LEDs to current load status
        ## fill data for leds
        for this_led in range(led.led_count):
            if (this_led <= LED_count_green):   ## current count is in the range of green LED to light
                green = 255
            else:
                green = 0
            if (this_led <= LED_count_red):     ## current count is also in the range of red LED to light
                red = 255
            else:
                red = 0

            blue = 0
            this_data = (red,green,blue)        ## combine the colors to a color tuple for the LED
            led.leds[this_led]=(this_data)      ## update data for the current LED
